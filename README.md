...
to S3 / OSS, and builds vector indexes. Doris uses `vector_search()` to retrieve similar clips, analyze
...
Install the PyTorch wheel appropriate for your CUDA/CPU environment. Edit `.env` to provide S3 / OSS
credentials and region (OSS also needs an endpoint), then load its variables:
...
does not create buckets: use an existing S3 / OSS bucket and credentials with read, list, and write
permissions. Refresh expired STS tokens in the environment before resuming.

## AWS S3

For AWS S3, set `AWS_REGION`, `AWS_ACCESS_KEY_ID`, and `AWS_SECRET_ACCESS_KEY` in `.env`
(plus `AWS_SESSION_TOKEN` for temporary credentials), then use an `s3://` output:
...
Export, resume, indexing, query-vector reads, and video extraction all support S3.
Use `doris/sql/01_setup_s3.sql` for Doris; set
`NOTEBOOK_DATASET_ROOT=s3://YOUR_BUCKET/robotics/droid100` for the notebook.
See [S3, compatible services, and OSS configuration](docs/STORAGE.md).
The commands below retain OSS examples; for S3, substitute your `s3://` prefix and AWS settings.
...
`work/local-output`; this does not require S3 / OSS credentials.
...
`_indices/` directory. These are not standalone S3 / OSS MP4 URLs. Vector Search does not require copying
...
1. Configure the bucket and credentials in `01_setup_s3.sql` (S3) or `01_setup.sql` (OSS), then create the Lance Catalog and analysis tables.
...
The code has not been run against your S3 / OSS/Doris environment.
